...
async def get_top_selling_products(year: int, db: Session = Depends(get_db)):
    ...
    products_query = (
        db.query(
            Product.product,
            sales_query.c.total_sales.label("total_sales")
        )
    ...
    result = [{"product_name": p.product, "total_sales": p.total_sales} for p in products_query]
...
def last_order_per_customer(db: Session = Depends(get_db)):
    subquery = (
        db.query(
            Sales.customer_id,
            func.max(Sales.transaction_date).label("last_order_date")
        )
