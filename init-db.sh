...
psql -U $POSTGRES_USER -d $POSTGRES_DB -c "CREATE TABLE sales(transaction_id int, transaction_date date, transaction_time time, sales_outlet_id int, staff_id int, customer_id int, instore_yn text, order_number int, line_item_id int, product_id int, quantity int, line_item_amount decimal, unit_price decimal, promo_item_yn text)"
...
psql -U $POSTGRES_USER -d $POSTGRES_DB -c "\copy sales from /csv/sales.csv delimiter ',' CSV HEADER"